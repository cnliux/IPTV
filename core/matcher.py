...
class AutoCategoryMatcher:
    ...
    def __init__(self, template_path: str, config=None):
        ...
        # 移除或注释掉 enable_debug 和 enable_space_clean，改为依赖日志级别
        # self.enable_debug = self.config.getboolean('MATCHER', 'enable_debug_classification', fallback=False)
        ...
        # 如果需要，可以通过日志级别控制调试信息，不需要单独的 enable_debug
    ...
    def batch_match(self, channel_names: List[str]) -> Dict[str, str]:
        ...
        batch_size = self.config.getint('PERFORMANCE', 'classification_batch_size', fallback=2000)
