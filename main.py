...
async def test_channels(tester: SpeedTester, channels: List[Channel], whitelist: Set[str], logger: logging.Logger) -> Set[str]:
    ...
    failed_urls = set()
    batch_size = min(200, len(channels) // 5 or 50) if os.name == 'nt' else min(1000, len(channels) // 10 or 100)
...
def export_results(exporter: ResultExporter, channels: List[Channel], whitelist: Set[str], logger: logging.Logger) -> None:
    """结果导出"""
    progress = SmartProgress(1, "导出进度")
    exporter.export(channels, whitelist, progress.update)
    progress.complete()
...
async def main():
    ...
    try:
        ...
        # ==================== 订阅源获取阶段 ====================
        logger.info("\n🔹🔹🔹🔹 阶段2/7：获取订阅源")
        fetcher = SourceFetcher(timeout=config.getfloat('FETCHER', 'timeout', fallback=15),
                                concurrency=config.getint('FETCHER', 'concurrency', fallback=5), config=config)
        ...
        # ==================== 智能分类阶段 ====================
        logger.info("\n🔹🔹🔹🔹 阶段5/7：智能分类")
        matcher = AutoCategoryMatcher(config.get('PATHS', 'templates_path', fallback='config/templates.txt'), config)
        ...
        # ==================== 测速测试阶段 ====================
        logger.info("\n🔹🔹🔹🔹 阶段6/7：测速测试")
        tester = SpeedTester(timeout=config.getfloat('TESTER', 'timeout', fallback=10),
                             ...
                             enable_logging=config.getboolean('TESTER', 'enable_logging', fallback=False), config=config)
        ...
        # ==================== 结果导出阶段 ====================
        logger.info("\n🔹🔹🔹🔹 阶段7/7：结果导出")
        exporter = ResultExporter(output_dir=config.get('MAIN', 'output_dir', fallback='outputs'),
                                  template_path=config.get('PATHS', 'templates_path', fallback='config/templates.txt'),
                                  config=config, matcher=matcher)
        export_results(exporter, sorted_channels, whitelist, logger)
