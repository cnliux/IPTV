...
async def main():
    ...
    try:
        ...
        for channel in all_channels:
            if channel.url not in unique_channels_dict:
                unique_channels_dict[channel.url] = channel
            else:
                duplicate_count += 1
        
        unique_channels = list(unique_channels_dict.values())
        logger.info(f"去重完成: 原始 {len(all_channels)} 个频道，去重后 {len(unique_channels)} 个频道，移除 {duplicate_count} 个重复项")
        ...
        for channel in unique_channels:
            if is_blacklisted(channel, blacklist):
                blacklisted_count += 1
            else:
                filtered_channels.append(channel)
        
        logger.info(f"黑名单过滤完成: 去重后 {len(unique_channels)} 个频道，过滤掉 {blacklisted_count} 个黑名单频道，剩余 {len(filtered_channels)} 个频道")

        # 进行垃圾回收
        if len(filtered_channels) % 500 == 0:
            gc.collect()
        # ========== 新增阶段结束 ==========

        # 阶段3: 智能分类与过滤
        matcher = AutoCategoryMatcher(str(templates_path), config)
        classify_progress = SmartProgress(len(filtered_channels), "🏷🏷🏷🏷 分类频道")
        ...
        uncategorized_count = sum(len(group) for group in uncategorized_groups.values())
        logger.info(f"分类完成: 已分类频道 {len(processed_channels)} 个, 未分类频道 {uncategorized_count} 个")
        ...
        if uncategorized_groups:
            ...
            with open(uncategorized_path, 'w', encoding='utf-8') as f:
                ...
                for category in sorted(uncategorized_groups.keys()):
                    ...
                    f.write("\n")
            logger.info(f"📝📝📝📝 未分类频道已保存到: {uncategorized_path}，共 {uncategorized_count} 个频道")
        else:
